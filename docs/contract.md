...
- **`catApp` 은 지금 신뢰할 수 없다.** 손으로 넣은 값이라 지역별로 기준이
  다르다. 예를 들어 제주·부산의 해변 26곳이 `sea` 가 아니라 `heal` 로 들어가
  있다. TourAPI 재분류(`pipeline/tourapi.py`)가 끝나면 `catOfficial` 필드가
  추가되며, 그때 어느 쪽을 쓸지 다시 합의한다
...
```json
{
  ...
  "tfi": {
    "경주": {"미식": 0.4208, "쇼핑": 0.0, "역사·문화유산": 1.0, "자연·힐링": 0.1955}
  },
  "detail": {
    "경주": {
      "share_pct": {},
...
| 필드 | 설명 |
|---|---|
| `themes` | 테마 축 4종. 순서가 의미를 갖지 않는다 |
| `regions` | TFI를 낸 지역 목록 |
| `tfi` | 지역×테마 강도, **0~1 정규화**. 추천 가중치로 쓴다 |
| `detail[].share_pct` | 정규화 전 원시 비중(%) |
| `detail[].age_consumption` | 성·연령별 소비 분포 |
| `detail[].flows` | 유입·유출 연관지역. 동선 그래프 간선 후보 |
| `detail[].source_files` | 이 값을 만든 CSV 파일명. 추적용 |

### 주의

- **`tfi` 값은 받은 지역들 사이의 상대값이다.** 전국 기준값이 아니다.
  지역이 1곳뿐이면 비교 기준이 없어 `null` 이 들어간다. `null` 을 0으로
  치환하지 말 것 — "약하다"가 아니라 "모른다"는 뜻이다
- 지역을 추가하면 **기존 지역의 tfi 값도 바뀐다.** 평균이 기준이기 때문이다.
  백엔드는 이 파일을 통째로 다시 읽어야 하며, 지역별로 부분 갱신하면 안 된다
- `share_pct` 의 미식·쇼핑은 **소비액 비중**, 역사·자연은 **장소 개수 비중**
  이라 단위가 다르다. 같은 축에 놓고 비교하지 말 것.
  (TFI는 테마별로 따로 정규화하므로 영향을 받지 않는다)
